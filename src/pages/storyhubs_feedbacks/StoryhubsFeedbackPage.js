...
const StoryhubsFeedback = (props) => {
      ...
      setStoryhubsFeedbacks((prevStoryhubsFeedbacks) => ({
        ...
        results: prevStoryhubsFeedbacks.results.map((storyhubs_feedback) => {
          return storyhubs_feedback.id === id
            ? { ...storyhubs_feedback, likes_count: storyhubs_feedback.likes_count + 1, like_id: data.id }
            : storyhubs_feedback;
        }),
      }));
    } catch (err) {
  ...
  const handleUnlike = async () => {
    try {
      ...
      setStoryhubsFeedbacks((prevStoryhubsFeedbacks) => ({
        ...
        results: prevStoryhubsFeedbacks.results.map((storyhubs_feedback) => {
          return storyhubs_feedback.id === id
            ? { ...storyhubs_feedback, likes_count: book_review.likes_count - 1, like_id: null }
            : storyhubs_feedback;
        }),
      }));
    } catch (err) {
  ...
  return (
    <Card className={styles.StoryhubsFeedback}>
      ...
      <Card.Body>
        {story_title && <Card.Title className="text-center">{story_title}</Card.Title>}
        {story_creator && <Card.Title className="text-center">{author}</Card.Title>}
        ...
        <div className={styles.PostBar}>
          {is_owner ? (
            <OverlayTrigger
              placement="top"
              overlay={<Tooltip>You won't be able to like your own Feedback!</Tooltip>}
            >
